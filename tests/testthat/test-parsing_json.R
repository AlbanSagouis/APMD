test_that("parsing_json returns a data.table", {
  result <- parsing_json(test_path("testdata", "nossaflex_Vito_70_test-week.txt"))
...
test_that("parsing_json Voigtlaender Vito 70 correction sets SS and A to 'auto'", {
  result <- parsing_json(
    test_path("testdata", "nossaflex_Vito_70_test-week.txt"),
...
test_that("parsing_frames returns a data.table", {
  result <- parsing_frames(test_path("testdata", "Berlin_Boat.frames"))
  expect_s3_class(result, "data.table")
})
